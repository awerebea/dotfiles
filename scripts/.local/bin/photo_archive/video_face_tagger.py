...
if __name__ == "__main__":
    ...
    # the importing process. Must happen before opencv or numpy are imported.
    bootstrap.reexec("video_face_tagger", Path(__file__).resolve())

from photo_archive import media, settings
from photo_archive.exif import ExifTool, Namespace
from photo_archive.media import (
    discover_videos,
    ffprobe_video,
    resolve_scope,
    sidecar_for,
...
VENV_DIR = bootstrap.venv_candidates("video_face_tagger")[0]
...
VIDFACES_NS = Namespace(
    prefix=VF_PREFIX,
    uri=VF_NAMESPACE_URI,
    tags=("FacesScanned", "FacesFound", "SourceVideo"),
    group2="Image",
)

EXIF = ExifTool(
    config_path=EXIFTOOL_CONFIG,
    namespace=VIDFACES_NS,
    generator="video_face_tagger.py",
)


def ensure_exiftool_config() -> Path:
    return EXIF.ensure_config()


def run_exiftool(
    args: Sequence[str], check: bool = False
) -> subprocess.CompletedProcess:
    return EXIF.run(args, check=check)


def exiftool_read(paths: Sequence[Path], tags: Sequence[str]) -> dict[Path, dict]:
    return EXIF.read(paths, tags)


def exiftool_write(
    target: Path, assignments: Sequence[str], dry_run: bool
) -> bool:
    return EXIF.write(target, assignments, dry_run)


def read_markers(videos: Sequence[Path]) -> dict[Path, str]:
    """Map video path -> marker value, for videos already carrying one."""
    return media.read_markers(EXIF, videos, MARKER_TAG)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    settings.check_dependencies(
        ("exiftool", "ffmpeg", "ffprobe"), "brew install exiftool ffmpeg"
    )
