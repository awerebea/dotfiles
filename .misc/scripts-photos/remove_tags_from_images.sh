...
while [ $i -lt "${#file_list[@]}" ]; do
    if [ -f "${file_list[$i]}.xmp" ]; then
        ...
        CREATOR_TOOL=""
        if exiftool "${file_list[$i]}" | grep -e "Creator Tool" | grep -e "photools.com IMatch" >/dev/null; then
        ...
        progress-bar $(( i + 1 )) ${#file_list[@]} \
            "$(
                ...
                -Comment= \
                $CREATOR_TOOL \
                -P -r -overwrite_original "${file_list[$i]}"
            )" | tee -a image_metadata_clean_up.log
    else
        echo "$(date +%Y-%m-%d_%H-%M-%S): ${file_list[$i]}" >> metadata_clean_up_skipped_files.log
        progress-bar $(( i + 1 )) ${#file_list[@]} \
            "Skip: \"${file_list[$i]/"$SOURCE_DIR"\//}\", because no matching .xmp file was found."
    fi
    i=$(( i + 1 ))
done
printf '\nFinished!\n'
